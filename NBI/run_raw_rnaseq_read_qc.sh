...
if [ "$gff" != "NA" ] && [ -h "gff.gff" ] && [ -f "gff.gff" ]; then
	if [ -h "fastq8.fq.gz" ] && [ -f "fastq8.fq.gz" ]; then
		...
		bwa mem genome.fa -t 16 fastq13579.fq.gz fastq246810.fq.gz -o sam.sam
		...
		bwa mem genome.fa -t 16 fastq1357.fq.gz fastq2468.fq.gz -o sam.sam
		samtools view -@16 -bS sam.sam > sam.bam
		samtools sort -@16 sam.bam -o sorted_sam.bam
		...
		bwa mem genome.fa -t 16 fastq135.fq.gz fastq246.fq.gz -o sam.sam
		samtools view -@16 -bS sam.sam > sam.bam
		samtools sort -@16 sam.bam -o sorted_sam.bam
