...
my %opts; getopts('q:N:n:c:t:j:i:m:d:er', \%opts); my $status = '--mail-type=END,FAIL'; my $email = '--mail-user=thomas.mathers@jic.ac.uk'; my $datestring = localtime(); my $hashes ='##########################'; 
unless ($opts{i}) {
...
# Collect job id then if failed -send me an email my $bashout = <STDIN>; my @bashout = split(/ /,$bashout); my $jobid = $bashout[3]; print STDOUT "this is the job id $jobid"; exit;
#
# afternotok:job_id[:jobid...] This job can begin execution after the specified jobs have terminated in some failed state (non-zero exit code, node failure, timed out, etc). afterok:job_id[:jobid...] This job can 
# begin execution after the specified jobs have successfully executed (ran to completion with an exit code of zero).
#
#unless ($opts{r}) {
